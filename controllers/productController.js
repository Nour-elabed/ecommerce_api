...
export const getAllProducts = async (req, res, next) => {
    try {
        ...
        if (total === 0 && !category && !brand && !gender && !search) {
            console.log("Database empty, attempting auto-seed...");
            try {
                // Roles must match the ROLES constants (uppercase) stored in the DB
                const adminUser = await User.findOne({ role: { $in: [ROLES.ADMIN, ROLES.SUPER_ADMIN] } });

                if (!adminUser) {
                    // Can't seed without a seller — Product.seller is required.
                    // The frontend will fall back to localProducts.ts seed data.
                    console.log("Auto-seed skipped: no admin user found. Use /api/setup to create one, then run 'npm run seed:products'.");
                } else {
                    const seedData = [
                    ...
                    const seededProducts = seedData.map(p => ({ ...p, seller: adminUser._id }));
                    await Product.insertMany(seededProducts);
                    console.log(`Auto-seeded ${seededProducts.length} products.`);
